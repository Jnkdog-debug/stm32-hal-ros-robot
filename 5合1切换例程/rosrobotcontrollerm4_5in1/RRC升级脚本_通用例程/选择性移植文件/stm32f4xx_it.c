...
void EXTI15_10_IRQHandler(void)
{
  /* USER CODE BEGIN EXTI15_10_IRQn 0 */
    ...
    if(__HAL_GPIO_EXTI_GET_IT(IMU_ITR_Pin) != RESET) {	//ÊÇ·ñÎªIMUÖÐ¶ÏÒý½ÅÐÅºÅ
        __HAL_GPIO_EXTI_CLEAR_IT(IMU_ITR_Pin);	//Çå³ýIMUÖÐ¶ÏÒý½ÅÐÅºÅ
		//·¢ËÍÐÅºÅÁ¿
...
//  HAL_UART_IRQHandler(&huart6);
    ...
    if(__HAL_UART_GET_FLAG(&huart6, UART_FLAG_TXE) != RESET) {
        ...
        if(serial_servo_controller.tx_byte_index <( (serial_servo_controller.tx_frame.elements.length + 3) ) ){  /* åˆ¤æ–­æ•°æ®æ˜¯å¦å‘é¿å®Œæˆ¿ */
            huart6.Instance->DR = ((uint8_t*)(&serial_servo_controller.tx_frame))[serial_servo_controller.tx_byte_index++]; /* ç»§ç»­å‘é¿ä¸‹ä¸¿ä¸ªå­—èŠ¿ */
        } else {
    ...
    if(__HAL_UART_GET_FLAG(&huart6, UART_FLAG_TC) != RESET) {
        ...
        HAL_GPIO_WritePin(SERIAL_SERVO_RX_EN_GPIO_Port, SERIAL_SERVO_RX_EN_Pin, GPIO_PIN_RESET);  /* è½¬å…¥æŽ¥æ”¶æ¨¡å¼ */
